...
function exitToMainMenu() {
    ...
    if (state.battleActive || state.floor > 1) {
        saveGame();
    }
...
function showScreen(screenId) {
    ...
    // Salva tela anterior se estiver abrindo as configurações
    if (screenId === 'settings') {
        if (state.screen !== 'settings') {
            previousScreen = state.screen || 'start';
        }
    }
    ...
    // Exibe ou oculta o botão flutuante de configurações conforme a tela
    const globalSettingsBtn = document.getElementById('global-settings-btn');
    if (globalSettingsBtn) {
        const gameplayScreens = ['battle', 'map', 'shop', 'campfire', 'reward', 'blessing', 'act-transition'];
        if (gameplayScreens.includes(screenId)) {
    ...
    // Exibe ou oculta o botão de menu principal nas configurações se vier de um jogo ativo
    const exitMenuBtn = document.getElementById('exit-menu-btn');
    if (exitMenuBtn) {
        if (screenId === 'settings' && previousScreen !== 'start') {
            exitMenuBtn.style.display = 'block';
        } else {
            ...
            el.onclick = () => {
                if (state.gold >= price) {
                    ...
                    if (item.type === 'relic') {
                        ...
                        state.player.deck.push({ ...item.card, fromShop: true });
                    }
...
function renderSkillTree() {
    ...
    // Renderiza a seção de Evolução de Cartas Iniciais
    const evolutionsContainer = document.getElementById('card-evolutions-container');
    if (evolutionsContainer) {
        ...
        for (let v = 1; v <= 10; v++) {
            ...
            cardEl.className = `evolution-card ${isEvolved ? 'evolved legendary' : 'common'}`;

            // Desenha a carta usando SVG do jogo!
            const svgContent = getCardGraphicSVG('basic', v, isEvolved ? 'legendary' : 'common');

            cardEl.innerHTML = `
                ${svgContent}
                <span class="card-value-display top-left" style="font-size: 0.85em; top: 6px; left: 8px;">${v}${isEvolved ? '+' : ''}</span>
                <div class="evolution-info" style="position: absolute; bottom: 8px; left: 0; width: 100%; text-align: center; display: flex; flex-direction: column; align-items: center; gap: 4px;">
                    <span style="font-size: 11px; font-weight: 700; color: ${isEvolved ? '#ffd700' : 'var(--text-muted)'}; text-shadow: 0 1px 3px black;">
                        ${isEvolved ? '✓ EVOLUÍDA' : '🌟 50 Poeira'}
                    </span>
                    <button class="menu-btn" style="padding: 4px 8px; font-size: 0.75em; margin: 0; min-width: 70px; border-radius: 4px;" 
                        ${isEvolved || state.starDust < cost ? 'disabled' : ''} 
                        onclick="evolveCard(${v})">
                        ${isEvolved ? 'MAX' : 'EVOLUIR'}
                    </button>
                </div>
            `;

            applyCardTilt(cardEl);
            evolutionsContainer.appendChild(cardEl);
        }
    }
}
...
function evolveCard(value) {
    const cost = 50;
    loadMetaProgression(); // Sincroniza do localStorage
    if (state.starDust >= cost) {
        state.starDust -= cost;
        if (!state.cardEvolutions) state.cardEvolutions = {};
        state.cardEvolutions[value] = 1;

        SoundManager.play('buy');
        saveMetaProgression();
        renderSkillTree();
        showToast(`Carta ${value} Evoluída com Sucesso!`, "#2ecc71");
    } else {
        showToast("Poeira Estelar insuficiente!");
    }
}
window.evolveCard = evolveCard;
...
function showEndRunScreen(victory = false) {
    ...
    // Calcula Poeira Estelar obtida (aumentada de 2 para 10 por andar)
    let dustEarned = (state.floor * 10);

    // Recompensa de 350 poeira ao concluir o mundo 3 com vitória
    if (victory && state.currentMap === 3) {
        dustEarned += 350;
    } else if (victory) {
        dustEarned += 100;
    }
...
function showActTransitionScreen(nextMap) {
    ...
    // Recompensa de conclusão de Mundo (Mundo 1 = 150, Mundo 2 = 250)
    let completionDust = 0;
    if (nextMap === 2) {
        completionDust = 150;
    } else if (nextMap === 3) {
        completionDust = 250;
    ...
    if (completionDust > 0) {
        const starDustLvl = state.skills.star_dust_multiplier || 0;
        const starDustBonus = starDustLvl > 0 ? SKILL_TREE.star_dust_multiplier.values[starDustLvl - 1] : 1.0;
        const finalDust = Math.floor(completionDust * starDustBonus);
        ...
        setTimeout(() => {
            showToast(`✨ Conclusão do Mundo: +${finalDust} Poeira Estelar!`, "#ffd700");
        }, 800);
    }
...
function openHeroSelector(mapNum) {
    showScreen('hero-select');
    renderHeroSelector(mapNum);
}

function renderHeroSelector(mapNum) {
    ...
    const heroes = [
        {
            ...
            name: "Guerreiro",
            icon: "🛡️",
            desc: "Especialista em defesa robusta e força bruta. Ideal para aguentar golpes pesados.",
            relic: "Escudo de Carvalho (🛡️)",
            relicDesc: "Começa cada batalha com 10 de Escudo.",
            relicId: "shield",
            cards: ["Espada Rúnica", "Muro de Aço", "Espinhos de Ferro"],
            cardObjs: [
                { value: 4, name: "Espada Rúnica", type: "attack", power: 12, rarity: "rare" },
                { value: 7, name: "Muro de Aço", type: "defend", power: 15, rarity: "rare" },
                { value: 5, name: "Espinhos de Ferro", type: "spikes", power: 4, rarity: "rare", effectText: "Espinhos +4" }
            ],
            bg: "linear-gradient(135deg, rgba(230, 57, 70, 0.15) 0%, rgba(150, 20, 30, 0.25) 100%)",
            borderColor: "#e63946"
        },
        {
            id: 'archer',
            name: "Arqueiro",
            icon: "🏹",
            desc: "Ágil e preciso, mestre do combate tático e venenos silenciosos.",
            relic: "Ampulheta (⏳)",
            relicDesc: "Uma vez por turno, pode descartar e comprar de novo.",
            relicId: "hourglass",
            cards: ["Tiro Rápido", "Mira Perfeita", "Adaga Tóxica"],
            cardObjs: [
                { value: 3, name: "Tiro Rápido", type: "attack", power: 10, rarity: "rare" },
                { value: 0, name: "Mira Perfeita", type: "wild", power: 4, rarity: "rare", effectText: "Qualquer slot" },
                { value: 3, name: "Adaga Tóxica", type: "poison", power: 3, rarity: "rare", effectText: "Veneno +3" }
            ],
            bg: "linear-gradient(135deg, rgba(46, 204, 113, 0.15) 0%, rgba(26, 120, 60, 0.25) 100%)",
            borderColor: "#2ecc71"
        },
        {
            id: 'mage',
            ...
            relicDesc: "Toda vez que você curar HP, causa 3 de dano ao inimigo.",
            relicId: "cauldron",
            cards: ["Bola de Fogo", "Explosão Arcana", "Tempestade Rúnica"],
            cardObjs: [
                { value: 6, name: "Bola de Fogo", type: "attack", power: 5, status: { burn: 3 }, rarity: "rare", effectText: "Burn: 3" },
                { value: 8, name: "Explosão Arcana", type: "attack", power: 16, rarity: "rare" },
                { value: 8, name: "Tempestade Rúnica", type: "storm", power: 3, rarity: "legendary", effectText: "Dano = 3x slots cheios" }
            ],
            bg: "linear-gradient(135deg, rgba(155, 89, 182, 0.15) 0%, rgba(100, 40, 130, 0.25) 100%)",
            borderColor: "#9b59b2"
        },
        {
            id: 'cleric',
            name: "Clérigo",
            icon: "☀️",
            desc: "Canalizador da luz divina, equilibrando milagres de cura e retaliação sagrada.",
            relic: "Pena de Fênix (🪶)",
            relicDesc: "Ressuscita uma vez por combate com 25% HP ao sofrer dano letal.",
            relicId: "phoenix_feather",
            cards: ["Prece de Cura", "Luz Sagrada", "Coringa de Bronze"],
            cardObjs: [
                { value: 8, name: "Prece de Cura", type: "heal", power: 18, rarity: "rare" },
                { value: 1, name: "Luz Sagrada", type: "all", power: 10, rarity: "legendary", effectText: "Dano + Cura" },
                { value: 0, name: "Coringa de Bronze", type: "wild", power: 3, rarity: "common", effectText: "Qualquer slot" }
            ],
            bg: "linear-gradient(135deg, rgba(241, 196, 15, 0.15) 0%, rgba(180, 140, 10, 0.25) 100%)",
            borderColor: "#f1c40f"
        },
        {
            id: 'oicani',
            name: "Oicani",
            icon: "👺",
            desc: "Um ladino sorrateiro das profundezas, mestre em trapaças e em usar recursos dos oponentes.",
            relic: "Mão do Ladrão (🧤)",
            relicDesc: "Clique em uma carta no tabuleiro inimigo para roubá-la (1x turno).",
            relicId: "thief",
            cards: ["Apunhalada", "Gatuno Sutil", "CORINGA"],
            cardObjs: [
                { value: 2, name: "Apunhalada", type: "attack", power: 12, rarity: "rare" },
                { value: 3, name: "Gatuno Sutil", type: "poison", power: 4, rarity: "rare", effectText: "Veneno +4" },
                { value: 0, name: "CORINGA", type: "wild", power: 5, rarity: "legendary", effectText: "Qualquer slot" }
            ],
            bg: "linear-gradient(135deg, rgba(39, 174, 96, 0.15) 0%, rgba(30, 130, 76, 0.25) 100%)",
            borderColor: "#27ae60"
        }
    ];
    ...
    heroes.forEach(h => {
        const card = document.createElement('div');
        card.className = `glass map-selector-card unlocked hero-card-${h.id}`;
        card.style.background = h.bg;

        card.onclick = () => {
            SoundManager.play('click');
            startNewRun(mapNum, h);
        };

        card.innerHTML = `
            <div class="map-icon" style="color: ${h.borderColor}; text-shadow: 0 0 10px ${h.borderColor}55; height: 50px; font-size: 2.8em; margin-bottom: 8px;">${h.icon}</div>
            <h3 class="cinzel" style="margin: 5px 0 10px 0; color: #fff; font-size: 1.25em; text-shadow: 0 0 5px rgba(255,255,255,0.2);">${h.name}</h3>
            <p style="font-size: 0.78em; color: var(--text-muted); margin: 0 0 12px 0; line-height: 1.45; text-align: center; height: 60px; overflow: hidden;">${h.desc}</p>
            
            <div style="font-size: 0.78em; border-top: 1px solid rgba(255,255,255,0.08); padding-top: 10px; margin-top: auto; text-align: left; width: 100%;">
                <div style="font-weight: 700; color: #ffd700; margin-bottom: 2px;">🎁 Relíquia Inicial:</div>
                <div style="color: var(--text); font-size: 0.95em; font-weight: bold;">${h.relic}</div>
                <div style="font-size: 0.82em; color: var(--text-muted); font-style: italic; margin-bottom: 12px; line-height: 1.3;">${h.relicDesc}</div>
                
                <div style="font-weight: 700; color: var(--primary); margin-bottom: 6px;">🃏 Cartas Especiais:</div>
                <div style="display: flex; gap: 4px; flex-wrap: wrap;">
                    ${h.cards.map(cName => `<span class="price-tag" style="background: rgba(212, 175, 55, 0.15); border: 1px solid var(--primary); color: #fff; font-size: 0.7em; padding: 2px 6px; border-radius: 4px; margin-top: 0;">${cName}</span>`).join('')}
                </div>
            </div>
        `;
