...
class CharacterViewer {
    ...
    init() {
        ...
        const backLight = new THREE.DirectionalLight(0x74b9ff, 0.4);
    ...
    loadModel(modelName) {
        ...
        // Rastreia o nome do modelo mais recente para evitar race conditions com carregamentos assíncronos
        this.latestModelName = modelName;
        const currentLoadName = modelName;
        ...
        loader.load(
            glbUrl,
            (gltf) => {
                if (this.latestModelName !== currentLoadName) return;
                ...
                const fallbackToSpriteAndProcedural = () => {
                    ...
                    textureLoader.load(
                        ...
                        (texture) => {
                            ...
                            const spriteMaterial = new THREE.SpriteMaterial({ map: texture, transparent: true, alphaTest: 0.1, color: 0xffffff });
                        ...
                        (err2) => {
                            ...
                            console.log("Tentando carregar imagem de erro padrão: imagens/erro_imagens.png");
                            
                            const errorSpriteUrl = "imagens/erro_imagens.png";
                            textureLoader.load(
                                errorSpriteUrl,
                                (errTexture) => {
                                    if (this.latestModelName !== currentLoadName) return;
                                    errTexture.encoding = THREE.sRGBEncoding;
                                    const aspect = errTexture.image.width / errTexture.image.height;
                                    const height = 1.50;
                                    const width = height * aspect;

                                    const spriteMaterial = new THREE.SpriteMaterial({ map: errTexture, transparent: true, alphaTest: 0.1, color: 0xffffff });
                                    const sprite = new THREE.Sprite(spriteMaterial);
                                    sprite.scale.set(width, height, 1);
                                    sprite.position.set(0, height / 2 - 0.2, 0);
                                ...
                                undefined,
                                (err3) => {
                                    console.error("Falha crítica: Não foi possível carregar a imagem de erro padrão:", errorSpriteUrl, err3);
                                }
                            );
                        }
                    );
    ...
    cleanupActiveModel() {
        if (this.activeGroup) {
            // Limpa e descarta todos os filhos do grupo ativo para evitar modelos duplicados na cena
...
export function loadPlayerModel(modelName) {
    if (!playerViewer) init3DScene();
    
    // Map class IDs to their correct asset names
    ...
    if (classMapping[modelName]) {
        assetName = classMapping[modelName];
    }
    
    playerViewer.loadModel(assetName);
}
