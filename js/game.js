...
function saveMetaProgression() {
    localStorage.setItem('jogo1a10_meta_save', JSON.stringify({
        ...
        cardEvolutions: state.cardEvolutions // Salva evoluções das cartas!
    }));
...
function saveGame() {
    localStorage.setItem('jogo1a10_save', JSON.stringify({
        ...
        playerClass: state.player.class // Salva a classe do jogador!
    }));
...
function continueRun() {
    ...
    if (saved) {
        ...
        state.player.class = saved.playerClass || 'warrior'; // Restaura a classe do jogador!
...
function startNewRun(mapNum = 1, selectedHero = null) {
    ...
    // Concede relíquia inicial do herói
    if (selectedHero && selectedHero.relicId) {
        state.relics.push(selectedHero.relicId);
        state.player.class = selectedHero.id;
    } else {
    ...
    for (let i = 0; i < 4; i++) {
        for (let v = 1; v <= 10; v++) {
            const isEvolved = state.cardEvolutions && state.cardEvolutions[v] > 0;
            const rarity = 'common';
            const powerIncrease = rarity === 'legendary' ? 5 : 3;
            state.player.deck.push({
                value: v,
                type: 'basic',
                name: isEvolved ? `${v}+` : v.toString(),
                power: isEvolved ? (8 + powerIncrease) : 8,
                rarity: rarity,
                upgraded: isEvolved
            });
        }
    ...
    // Adiciona as 3 cartas especiais do herói
    if (selectedHero && selectedHero.cardObjs) {
        selectedHero.cardObjs.forEach(c => {
            state.player.deck.push({ ...c, upgraded: false });
        });
    }
...
function initBattle(isBoss = false) {
    state.player.hand = []; // Reseta a mão do jogador
    const deckCards = [...state.player.deck].map(c => ({ ...c }));
    const drawPileCards = [];
    
    deckCards.forEach(card => {
        const isGrimoire = card.fromShop === true || card.type !== 'basic' || card.upgraded === true || card.rarity !== 'common';
        if (isGrimoire) {
            state.player.hand.push(card);
        } else {
            drawPileCards.push(card);
        }
    });

    state.player.drawPile = drawPileCards.sort(() => Math.random() - 0.5);
    ...
    const modelName = selected.name.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    state.enemy.model = modelName;
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            if (playerStarts) {
                state.canPlay = true;
                state.playerBlockedDrawThisTurn = false;
                drawFromDeck();
            } else {
                endPlayerTurn();
            }
...
function renderBattle() {
    ...
    if (endTurnBtn) {
        endTurnBtn.disabled = !state.canPlay;
    }
    ...
    const grimoireSlot = document.getElementById('grimoire-slot');
    if (handSlot) handSlot.innerHTML = '';
    if (grimoireSlot) grimoireSlot.innerHTML = '';

    let hasNormalCards = false;
    let hasGrimoireCards = false;

    if (state.player.hand && state.player.hand.length > 0) {
        state.player.hand.forEach(card => {
            const cardEl = createCardElement(card);
            if (state.player.selectedCard === card) {
            ...
            cardEl.onclick = (e) => {
                ...
                if (state.player.selectedCard === card) {
                    state.player.selectedCard = null; // Deselecionar
                } else {
                    state.player.selectedCard = card; // Selecionar
                }
                renderBattle();
            };
            ...
            if (isGrimoire) {
                if (grimoireSlot) {
                    grimoireSlot.appendChild(cardEl);
                    hasGrimoireCards = true;
                }
            } else {
                if (handSlot) {
    ...
    if (!hasNormalCards && handSlot) {
        handSlot.innerHTML = `<span style="color: rgba(255, 255, 255, 0.3); font-size: 0.9em; font-style: italic; pointer-events: none;">Mão Vazia</span>`;
    }
    if (!hasGrimoireCards && grimoireSlot) {
        grimoireSlot.innerHTML = `<span style="color: rgba(255, 255, 255, 0.25); font-size: 0.85em; font-style: italic; pointer-events: none;">Vazio</span>`;
    }

    const braceletBtn = document.getElementById('lucky-bracelet-btn');
    if (braceletBtn) {
        const hasBracelet = state.relics.includes('bracelet');
        const canUse = hasBracelet && !usedBraceletThisTurn && state.canPlay && state.battleActive;
        const grimoireCards = state.player.hand.filter(c => (c.fromShop === true || c.type !== 'basic' || c.upgraded === true || c.rarity !== 'common'));
        
        if (canUse && grimoireCards.length > 0) {
            braceletBtn.style.display = 'block';
        } else {
            braceletBtn.style.display = 'none';
        }
    }
    ...
    // Inicia/Atualiza o modelo 3D do inimigo
    if (window.loadEnemyModel && window.currentLoadedModel !== state.enemy.model) {
        window.currentLoadedModel = state.enemy.model;
        window.loadEnemyModel(state.enemy.model);
    }

    // Inicia/Atualiza o modelo 3D do jogador
    const pClass = state.player.class || 'warrior';
    if (window.loadPlayerModel && window.currentLoadedPlayerModel !== pClass) {
        window.currentLoadedPlayerModel = pClass;
        window.loadPlayerModel(pClass);
    }
    ...
    // Atualiza dinamicamente a ação/dano da carta selecionada do jogador
    const playerIntentDisplay = document.getElementById('player-intent-display');
    const playerIntentText = document.getElementById('player-intent-text');
    const playerIntentIcon = playerIntentDisplay ? playerIntentDisplay.querySelector('.intent-icon') : null;

    if (playerIntentDisplay && playerIntentText) {
        if (state.player.selectedCard) {
            const card = state.player.selectedCard;
            playerIntentDisplay.style.display = 'flex';
            
            let icon = '⚔️';
            let text = `Dano: ${card.power}`;
            
            if (card.type === 'defend') {
                icon = '🛡️';
                text = `Defesa: ${card.power}`;
            } else if (card.type === 'heal') {
                icon = '❤️';
                text = `Cura: ${card.power}`;
            } else if (card.type === 'spikes') {
                icon = '🥊';
                text = `Espinhos: ${card.power}`;
            } else if (card.type === 'poison') {
                icon = '🧪';
                text = `Veneno: ${card.power}`;
            } else if (card.type === 'storm') {
                icon = '⚡';
                text = `Tempestade: ${card.power}`;
            } else if (card.type === 'wild') {
                icon = '✨';
                text = `Coringa: ${card.power}`;
            } else if (card.type === 'all') {
                icon = '☀️';
                text = `Misto: ${card.power}`;
            }
            
            if (playerIntentIcon) playerIntentIcon.innerText = icon;
            playerIntentText.innerText = text;
        } else {
            playerIntentDisplay.style.display = 'none';
        }
    }
}
...
function stealEnemyCard(index) {
    ...
    if (state.player.hand.length >= getHandLimit()) {
        showToast("Sua mão está cheia!");
        return;
    }

    SoundManager.play('draw');
    const stolenCard = { value: val, type: 'basic', name: val.toString(), power: 2, rarity: 'rare' };
    state.player.hand.push(stolenCard);
    if (!state.player.selectedCard) {
        state.player.selectedCard = stolenCard;
    }
    usedThiefHand = true;
...
function createCardElement(card) {
    ...
    if (card.isWildTemporary) {
        el.classList.add('wild-temp');
    }
    ...
    if (spriteName) {
        ...
        img.onerror = () => {
            if (img.src && !img.src.includes('erro_imagens.png')) {
                // Se falhar o carregamento da imagem original da carta, usa a imagem de erro padrão
                img.src = 'imagens/erro_imagens.png';
            } else {
                img.style.display = 'none'; // Fallback de segurança se até o erro_imagens falhar
            }
        };
...
function triggerCauldronEffect() {
    if (state.relics.includes('cauldron') && state.battleActive && state.enemy && state.enemy.hp > 0) {
        ...
        const enemySprite = document.getElementById('enemy-3d-container') || document.getElementById('enemy-sprite');
        ...
        if (window.triggerDamageAnimation) window.triggerDamageAnimation();
        renderBattle();
    }
}
...
function getHandLimit() {
    return 1 + (state.skills.hand_size || 0);
}

function discardDuplicatesInHand() {
    let discardedAny = false;
    for (let i = state.player.hand.length - 1; i >= 0; i--) {
        const hCard = state.player.hand[i];
        if (hCard.type !== 'wild' && state.player.board[hCard.value - 1] !== 0) {
            state.player.hand.splice(i, 1);
            state.player.discardPile.push(hCard);
            if (state.player.selectedCard === hCard) {
                state.player.selectedCard = null;
            }
            showToast(`Descartada: ${hCard.name} (Slot ocupado)`, "#e67e22");
            discardedAny = true;
        }
    }
    if (discardedAny) {
...
function drawFromDeck() {
    if (!state.battleActive || !state.canPlay) return;

    // Remove any cards from hand whose corresponding board slot is already occupied
    discardDuplicatesInHand();

    // Check space limits
    const grimoireLimit = state.skills.hand_size || 0;
    
    const normalInHand = state.player.hand.filter(c => !(c.fromShop === true || c.type !== 'basic' || c.upgraded === true || c.rarity !== 'common'));
    const grimoireInHand = state.player.hand.filter(c => (c.fromShop === true || c.type !== 'basic' || c.upgraded === true || c.rarity !== 'common'));

    const needsNormal = normalInHand.length < 1 && !state.playerBlockedDrawThisTurn;
    const needsGrimoire = grimoireInHand.length < grimoireLimit;

    if (!needsNormal && !needsGrimoire) {
        if (!state.player.selectedCard && state.player.hand.length > 0) {
            state.player.selectedCard = state.player.hand[0];
        }
        renderBattle();
        return;
    }
    ...
    // Search the draw pile for a card we can hold (from top to bottom)
    let cardIndex = -1;
    for (let i = state.player.drawPile.length - 1; i >= 0; i--) {
        const card = state.player.drawPile[i];
        const isCardGrimoire = card.fromShop === true || card.type !== 'basic' || card.upgraded === true || card.rarity !== 'common';
        
        if (isCardGrimoire && needsGrimoire) {
            cardIndex = i;
            break;
        } else if (!isCardGrimoire && needsNormal) {
            cardIndex = i;
            break;
        }
    }
    ...
    if (cardIndex === -1) {
        // No valid card can be drawn at the moment
        if (!state.player.selectedCard && state.player.hand.length > 0) {
            state.player.selectedCard = state.player.hand[0];
        }
        renderBattle();
    ...
    const card = state.player.drawPile.splice(cardIndex, 1)[0];
    SoundManager.play('draw');
    ...
    if (card.type !== 'wild' && state.player.board[card.value - 1] !== 0) {
        if (state.relics.includes('hourglass') && !usedHourglass) {
            ...
            state.player.discardPile.push(card);
            state.playerBlockedDrawThisTurn = true;
            renderBattle();
        }
    } else {
        state.player.hand.push(card);
        ...
        // Recursively try to draw more cards if needs are not satisfied
        setTimeout(() => {
            drawFromDeck();
        }, 200);
    }
...
function handleSlotClick(index) {
    ...
    if (card.type === 'wild' || card.isWildTemporary || card.value === pos) {
        ...
        state.player.board[index] = (card.type === 'wild' || card.isWildTemporary ? pos : card.value);
        state.player.boardCards[index] = { ...card }; // Guarda objeto completo para sprite no tabuleiro
        delete state.player.boardCards[index].isWildTemporary; // Remove o estado coringa temporário ao ser colocada
        ...
        const cardIndex = state.player.hand.indexOf(card);
        if (cardIndex !== -1) {
            state.player.hand.splice(cardIndex, 1);
        }
        state.player.selectedCard = null;

        // Ao ser colocado, as cartas do grimório voltam ao normal
        state.player.hand.forEach(c => {
            if (c.isWildTemporary) {
                delete c.isWildTemporary;
            }
        });
        
        // Discard any cards in hand that have become duplicates due to this move
        discardDuplicatesInHand();
...
function applyCardEffect(card, isLucky = false) {
    ...
    // Trigger de animação de ataque 3D para o jogador
    if (['attack', 'storm', 'all'].includes(card.type) || card.type === 'basic') {
        if (window.triggerPlayerAttackAnimation) window.triggerPlayerAttackAnimation();
    }
...
function endPlayerTurn() {
    ...
    setTimeout(() => {
        ...
        if (state.enemy.intent.type === 'attack') {
            ...
            if (window.triggerAttackAnimation) window.triggerAttackAnimation();
            ...
            state.player.hp -= dmg;
            if (window.triggerPlayerDamageAnimation && dmg > 0) window.triggerPlayerDamageAnimation();
...
function enemyDrawCard() {
    ...
    if (state.enemy.board[idx] === 0) {
        state.enemy.board[idx] = val;
        SoundManager.play('click');

        // Cada carta revelada do inimigo causa dano ao jogador igual ao valor da carta (absorvido pelo escudo)
        const playerUI = document.querySelector('.battle-footer .info');
        const playerRect = playerUI ? playerUI.getBoundingClientRect() : { left: 0, top: 0 };
        const dmg = val;
        const actualDmg = Math.max(0, dmg - state.player.shield);
        state.player.shield = Math.max(0, state.player.shield - dmg);
        state.player.hp -= actualDmg;

        spawnFloatingText(actualDmg > 0 ? `-${actualDmg}💥` : "🛡️ Bloqueado", "#e63946", playerRect.left + 50, playerRect.top);
        if (window.triggerPlayerDamageAnimation && actualDmg > 0) window.triggerPlayerDamageAnimation();
        ...
        showToast("Turno do Inimigo: Carta Repetida!", "#e67e22");
        setTimeout(() => {
            usedHourglass = false;
            usedThiefHand = false;
            usedBraceletThisTurn = false;
...
function checkEnemyUltimate() {
    if (state.enemy.board.every(v => v !== 0)) {
        ...
        state.player.hp -= 20;
        if (window.triggerPlayerDamageAnimation) window.triggerPlayerDamageAnimation();
...
function victory() {
    if (window.triggerDeathAnimation) window.triggerDeathAnimation();
    ...
    if (isBossFight) {
        ...
        // Recompensa de poeira estelar por batalha comum ganha (aumentada)
        const baseDust = 15;
        ...
        const finalDust = Math.floor(baseDust * starDustBonus);

        loadMetaProgression();
        state.starDust += finalDust;
        saveMetaProgression();

        showToast(`✨ Vitória: +${finalDust} Poeira Estelar!`, "#2ecc71");
...
function checkDefeat() {
    if (state.player.hp <= 0) {
        ...
        if (window.triggerPlayerDeathAnimation) window.triggerPlayerDeathAnimation();
        state.battleActive = false;
        setTimeout(() => {
...
// --- BRACELETE DA BOA SORTE FUNCTION ---
function useLuckyBracelet() {
    if (!state.battleActive || !state.canPlay || usedBraceletThisTurn || !state.relics.includes('bracelet')) return;
    
    const grimoireCards = state.player.hand.filter(c => (c.fromShop === true || c.type !== 'basic' || c.upgraded === true || c.rarity !== 'common'));
    if (grimoireCards.length === 0) {
        showToast("Nenhuma carta no grimório para transformar!");
        return;
    }
    
    SoundManager.play('ultimate');
    grimoireCards.forEach(c => {
        c.isWildTemporary = true;
    });
    usedBraceletThisTurn = true;
    
    showToast("✨ Bracelete: Cartas do Grimório viraram Coringas!", "#2ecc71");
    spawnFloatingText("✨ CORINGAS!", "#2ecc71");
    renderBattle();
}
window.useLuckyBracelet = useLuckyBracelet;
